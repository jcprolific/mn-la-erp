...
1. **Warehouse Inventory In**
   ...
   - **Single write to `inventory`:** either `UPDATE inventory SET quantity = quantity + p_quantity` for the existing row, or `INSERT` one row if none exists. Then **one** `INSERT` into `inventory_movements` with `source = 'warehouse_inventory_in'`.
...
4. **Idempotency for store receive**
   - Do not remove the `request_id` check in `store_receive_inventory_v2`. Do not drop the unique index on `request_id` (where not null) on `inventory_movements`. Retries with the same `request_id` must not create duplicate movements or duplicate quantity.
...
1. Re-apply (in order):
   - `inventory_triggers.sql` (supporting schema: request_id, batch_id, unique constraint, movement_type check).
   - `warehouse_receive_inventory.sql`.
